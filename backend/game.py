...
from database import save_game_session, load_game_session
...
Generate a unique conflict scenario for level {level} of Peacecraft. 
The player must resolve it peacefully using negotiation and de-escalation. 
Ensure the situation increases in complexity as levels progress.
Make sure the conflict is engaging and realistic.
"""

NPC_RESPONSE_TEMPLATE = """
You are an NPC in the Peacecraft game. 
...
- Respond in a way that escalates or de-escalates depending on player choices.
- Keep responses natural and within the scope of the scenario.
"""
...
def generate_scenario(level):
    ...
    prompt = SCENARIO_PROMPT_TEMPLATE.format(level=level)  # Inject level into prompt

    try:
        response = requests.post(
            OLLAMA_API_URL,
            json={"model": "mistral-nemo", "prompt": prompt},
            stream=True,
        )
...
def get_npc_response(session_id, npc_personality, player_input, scenario):
    ...
    try:
        ...
        # Store the response in NPC memory
...
def start_game():
    ...
    existing_session = load_game_session(session_id)
    if existing_session:
        print("\n🔄 Resuming previous game...")
        level = existing_session["level"]  # ✅ Load last saved level
        npc_personality = existing_session["npc_personality"]
        scenario = existing_session["scenario"]
        trust_level = existing_session["trust_level"]
        ...
        conversation = []  # Empty conversation log
    ...
    # ✅ Keep looping through levels until game ends
    while level <= 4:
        ...
        game_status = play_game(
            session_id, level, npc_personality, scenario, trust_level, conversation
        )
        ...
        if game_status == "win":
            ...
            if level > 4:
                print(
                    "\n🎉 YOU HAVE COMPLETED PEACECRAFT! 🕊️ Congratulations on mastering conflict resolution!"
                )
                break  # End game after final level
            print(f"\n✅ Level {level - 1} complete! Moving to Level {level}...")
            npc_personality = random.choice(
                list(NPC_PERSONALITIES.keys())
            )  # New NPC personality
            scenario = generate_scenario(level)  # New scenario
            trust_level = 50  # Reset trust level for new level
            conversation = []  # Reset conversation for new level
        else:
...
def play_game(session_id, level, npc_personality, scenario, trust_level, conversation):
    ...
    while True:
        ...
        conversation.append(f"NPC ({npc_personality}): {npc_response}")
        ...
        # ✅ Game logic for level progression
        if trust_level > 65:
            print("\n✅ Congratulations! You successfully de-escalated the situation.")
            return "win"  # ✅ Move to next level
        elif trust_level < 25:
            print("\n❌ Game over. The conflict escalated beyond control.")
            return "loss"  # ❌ Restart game
        else:
