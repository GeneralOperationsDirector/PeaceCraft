...
async def start_game(request: Request):
    try:
        data = await request.json()  # Explicitly parse JSON
        print(
            f"Received JSON from Flask: {json.dumps(data, indent=4)}"
        )  # Debugging output
        ...
        # Generate conflict scenario dynamically
        scenario_prompt = (
            f"This is a game called Peacecraft, where players resolve conflicts peacefully.\n"
            f"The player is currently on Level {level}.\n"
            f"Generate an appropriate conflict scenario that matches this level, where the player must resolve the situation."
        )

        try:
            ollama_response = ollama.generate(
                model="mistral-nemo", prompt=scenario_prompt
            )
            scenario = ollama_response.response.strip()
            print(f"Generated Scenario: {scenario}")
        except Exception as e:
            print(f"Error generating scenario: {str(e)}")
            raise HTTPException(
                status_code=500, detail=f"Error generating scenario: {str(e)}"
            )
        ...
        print(f"Error parsing JSON: {str(e)}")  # Debugging output
...
def process_response(player_input: PlayerInput):
    ...
    try:
        ollama_response = ollama.generate(model="mistral-nemo", prompt=npc_prompt)
        npc_response = ollama_response.response.strip()
